...
class Transformer1D(BaseModule):
    ...
    class Config(BaseModule.Config):
        ...
        # --------------------
        use_lora: bool = False         # 新增 LoRA 控制參數
        lora_rank: int = 4            # LoRA 秩
        lora_alpha: float = 1.0       # LoRA 縮放因子
        # --------------------
        ...
        self.transformer_blocks = nn.ModuleList(
            [
                BasicTransformerBlock(
                    ...
                    # ---------------------
                    use_lora=self.cfg.use_lora,       # 傳遞 LoRA 控制參數
                    lora_rank=self.cfg.lora_rank,     # 傳遞 LoRA 秩
                    lora_alpha=self.cfg.lora_alpha,   # 傳遞 LoRA 縮放因子
                    # ---------------------
                )
